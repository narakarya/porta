  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[16px] font-semibold text-ink">Tailscale</h1>
      ...
      {status && !status.installed && (
        <div className="flex flex-col gap-3 px-3 py-3 rounded-card bg-[rgba(251,191,36,0.06)] border border-[rgba(251,191,36,0.2)]">
          ...
          <div className="flex items-center gap-2">
            <code className="flex-1 text-[11px] font-mono bg-surface-code px-2.5 py-1.5 rounded-control">
      ...
      {status?.installed && status?.running && status?.logged_in && (
        <div>
          <div className="flex items-center justify-between mb-2">
            <p className="text-[11px] font-medium text-ink-2 uppercase tracking-wide">Active Serves</p>
            <span className="text-[10px] text-ink-3">{serves.length} entries</span>
          </div>
          {serves.length === 0 && !loading && (
            <p className="text-[11px] text-ink-3 italic">No serves active. Start one from an app's Tunneling tab.</p>
          )}
          <div className="flex flex-col gap-2">
            {serveWithApp.map((s) => (
              <div
                key={`${s.port}-${s.upstream}`}
                className="flex items-start gap-3 px-3 py-2.5 rounded-card bg-surface-1 border border-subtle"
              >
                <div className={`shrink-0 w-2 h-2 rounded-full mt-1.5 ${s.funnel ? "bg-warn" : "bg-ok"}`} title={s.funnel ? "Public (Funnel)" : "Private (Serve)"} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <p className="text-[12px] font-medium text-ink truncate">
                      {s.appName ?? `Port ${s.port}`}
                    </p>
                    <span className={`text-[9px] px-1.5 py-0.5 rounded-control font-medium ${s.funnel ? "bg-warn-bg text-warn" : "bg-ok-bg text-ok"}`}>
      ...
      {status?.installed && serves.length > 0 && (
        <div className="flex items-center justify-between gap-3 p-3 rounded-card bg-surface-1 border border-subtle">
          <div>
            <p className="text-[11px] font-medium text-ink">Stop all Porta tunnels</p>
            <p className="text-[10px] text-ink-3 leading-relaxed mt-0.5">
        ...
        <div className="flex flex-col gap-2 p-3 rounded-card bg-[rgba(248,113,113,0.04)] border border-[rgba(248,113,113,0.2)] mt-2">
          ...
          {!confirmReset ? (
            ...
            <div className="flex gap-2 items-center">
              <span className="text-[11px] text-ink-2">Are you sure?</span>
              <button
              ...
              <button
                onClick={handleReset}
                disabled={resetting}
                className="px-3 py-1.5 text-[11px] font-medium bg-bad hover:opacity-90 text-white rounded-control disabled:opacity-50 transition-colors"
              >

...
import { detectStartCommand, nextAvailablePort, listAvailableCommands, parseComposeString } from "../../lib/commands";
...
export default function AddAppModal({ workspaceId, onClose, defaultValues }: Props) {
  ...
  async function submit(e: FormEvent) {
    ...
    if (!name) return;
    if (kind !== "docker" && kind !== "compose" && !rootDir) return;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Folder picker — required for non-docker apps, optional for docker (used as base for relative volume paths) */}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex gap-1 bg-[#111113] border border-white/[0.08] rounded-lg p-1">
            {(["process", "static", "docker", "compose"] as AppKind[]).map((k) => (
              <button
                key={k}
                type="button"
                onClick={() => setKind(k)}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {composeMode === "file" ? (
              <>
                ...
                />
                <span className="text-[11px] text-zinc-500">
                  Relative to Compose Project Folder. Absolute paths also OK.
                </span>
              </>
            ) : (
              <>

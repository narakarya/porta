  ...
  return (
    <div className="p-3 rounded-lg bg-blue-500/[0.06] border border-blue-500/20 flex flex-col gap-2">
      ...
      <button
        ...
        onClick={onRecheck}
        disabled={loading}
        className="self-start flex items-center gap-1.5 px-3 py-1 text-[11px] font-medium rounded-md bg-blue-500/10 hover:bg-blue-500/20 text-blue-300 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
      >
        {loading ? (
      ...
      {hint && (
        <p className="text-[10px] text-amber-400 leading-snug">{hint}</p>
      )}
    </div>
  );
}

...
export default function EditorSearchBar({
  ...
  return (
    <div className="flex items-center gap-1 bg-surface-0 border border-white/[0.12] rounded-md px-1.5 py-1 focus-within:border-[var(--accent)] transition-colors">
      <input
        ref={inputRef}
        type="text"
        value={query}
        ...
        placeholder="Find…"
        spellCheck={false}
        className="w-40 bg-transparent text-[12px] font-mono text-ink placeholder-ink-3 focus:outline-none px-1"
      />
      <span className="text-[10px] font-mono text-ink-3 tabular-nums min-w-[34px] text-right select-none">{counter}</span>
      <button type="button" onClick={onPrev} disabled={matchCount === 0} title="Previous (Shift+Enter)"
        className="p-1 text-ink-3 hover:text-ink hover:bg-white/[0.06] rounded disabled:opacity-30 transition-colors">

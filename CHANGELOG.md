...
### Added
- SSH host manager: attach hosts to workspaces (many-to-many) — vault filter by workspace, workspace badges on host cards, and a "Hosts" quick-connect section in each workspace view.
- Host form: "Browse…" native file picker for the SSH key and a default ~/.ssh/id_ed25519 path.
...
- **SSH Host Manager.** A new "Hosts" sidebar entry adds a saved-host vault
  ...
  SSH terminals over a pure-Rust `russh` transport â multiple concurrent
...
- **Instance cards get the full right-click menu, scoped to the instance.** The
  ...
  the parent app â Copy URL/host use the instance host, Open in Editor/Terminal
...
- **Git badge fully hidden on non-process apps.** 0.7.33 removed the branch
  ...
  process apps â including worktree instance cards.
...
- **Branch workflow hidden on apps that can't use it.** "Switch branch" and
  ...
  with a start command â the backend's `start_instance` rejects docker,
...
- **Tunnel icon on app cards stays visible while it has something to say.**
  ...
  after connecting â then returns to hover-only.
...
- **Quick-tunnel URLs no longer surface before they resolve
  ...
  cloudflared printed it, but the hostname is brand-new and local resolvers â
  especially VPN/Tailscale MagicDNS paths â can lag behind, so clicking the
...
- **Tunneled pages 502'd on every request after 0.7.28/0.7.29 (regression).**
  ...
  IPv6-first resolution race â correct for the TCP dial, but Go sends **no SNI**
...
- **Intermittent 502s from ghost tunnel connections at the Cloudflare edge.**
  ...
  requests across **all** registered connections â dead ones included â so pages
  ...
  the fresh connector â every connect self-heals the edge state. Full
...
- **Connecting a Cloudflare tunnel from an app card is now as reliable as from
  ...
  fell back to the store's in-memory `tunnel_provider` â which could be stale
...
- **Cloudflare named tunnels shared across apps now route correctly.** When two
  ...
  Cloudflare edge forwarded a hostname to whichever replica answered â and on
...
- **Intermittent "Bad Gateway" (502) on tunneled pages that a refresh cleared.**
  ...
  `127.0.0.1` only â so Caddy attempted `::1` first and Go's dual-stack fallback
...
- **Quick (trycloudflare) tunnels that provisioned a URL but 502'd on access** â
...
- **Stop no longer removes an instance.** Stopping an instance now kills its
  ...
  card stays put â mirroring how a stopped primary app behaves. Removal is a
  separate, explicit action.
- **Instance cards gain Kill port + Remove.** A stopped instance shows
  **Start Â· Kill port Â· Remove**. *Kill port* frees the instance's *own* port
...
- **Terminal runs a login+interactive shell** (`zsh -i -l`, was `-i`) so
  ...
  puts `/opt/homebrew/bin` on PATH â without it, `starship` isn't found and the
  prompt silently falls back to bare zsh (notably in worktree terminals).

### Added
- `remove_instance` command â the destructive counterpart to `stop_instance`.

### Fixed
- **Remove now asks for confirmation** â a confirm bar on instance cards and a
  two-step `Remove â Confirm?` in the branch flyout, so an instance isn't wiped
...
- **Branch flyout opens on hover.** The "Switch branch" row now reveals the
  branch list on hover â no click needed â with a short grace delay on leave so
...
- **Real tooltips replace native `title=`** across the app-card and GitBadge
  ...
  status, port-conflict badges, root-dir path, cleanup buttons) â portal-based,
...
- **Branch switching moves to a flyout.** The GitBadge now leads with a git
  ...
  second popover to the *right* â a searchable branch list where you click a
...
- **"Run from worktree" launcher shows 5 entries by default** with a "Show N
  moreâ¦" toggle (search still reveals all; worktrees with an active instance
  always stay visible).
...
- **GitBadge branch names auto-scroll on hover** instead of hard-truncating,
  and the popover header drops the `â upstream` arrow for a cleaner look.
...
- **Directional git-op loading in GitBadge.** Fetch spins a sync icon, Pull
  ...
  upward â the loading animation now mirrors each operation's direction.
...
- **Stale instance rows are reconciled on boot** â instances left
...
- **Starting, stopping, or restarting an app froze the window.** The lifecycle
  ...
  waited inline on `docker compose down`/`up -d` â several seconds during which
...
- **The git poller stopped re-scanning folders that aren't repos.** It ran
  ...
  pass â cutting most of the poller's background process churn on setups with
...
- **Fetch, Pull, and Push froze the whole window.** The git network commands ran
  synchronously on the main thread, so the WebView locked up until git returned â
...
- **A repo Porta couldn't read looked like it wasn't a repo at all.** `git status`
  exits 128 both for "not a git repository" and for real problems â `detected
  ...
  badge, with no way to tell why. It now shows `git â ` on the card; clicking it
...
- **On a git built with translations, every ordinary folder would have warned.**
  ...
  Homebrew's git â unlike Apple's â localizes it. Porta now runs `git status`
  under `LC_ALL=C`.
- **A hung remote could freeze the ahead/behind counts for minutes.** The
  ...
  behind an unreachable remote stalled every card's `âN` for up to four minutes.
...
- **A panic anywhere could silently kill a background poller for the rest of the
  ...
  thread â no CPU readings, no git status, no auto-sleep, and no sign anything
...
- **App cards show git state instead of resource numbers.** A card whose folder
  is a git repo now displays its branch, plus `âN` for commits waiting to be
  pushed and `âN` for commits waiting to be pulled. Unlike the badges it
  replaces, it is visible at rest rather than on hover â a repo that needs a
...
- **Background autofetch, so `âN` tells the truth.** Git can only know how far
  ...
  `git fetch --no-tags --prune` on an interval â never touching your working
  tree, index, or HEAD. Settings â Git has the toggle and the interval (1, 3, 5,
...
- **Resource monitoring moved to its own drawer (`ââ§M`).** It lists every app in
  the workspace with CPU, memory, and a rolling CPU sparkline; container rows
  expand to show network and disk I/O â the same figures the old per-card
  tooltip carried. Polling stops entirely when the drawer is closed.

### Removed
- **Three badges are gone from the app card**: the CPUÂ·memory readout (now in the
  ...
  start-order number is removed outright, not relocated; what determined it â
  each app's `depends_on` â remains editable in App Settings.
...
- **Secret values are blanked on copy, and empty values are flagged.** Keys that
  look like credentials (`SECRET`, `PASSWORD`, `TOKEN`, `API_KEY`, â¦) come across
  with an empty value, while defaults worth keeping â `API_URL`, `AUTH_URL`,
  `SIGNUP_ENABLED` â are preserved. Any variable with an empty value gets an
...
- **Setup persistently failed with "mkcert couldn't read its CA key (permission
  ...
  CA key there â so cert generation failed every time, and the 0.7.6 retry
...
### Fixed
- **Setup could fail with a one-off "mkcert generate failed: â¦ permission
  ...
  retry. `generate_certs` now retries the mkcert run up to 3Ã on a transient
  permission-denied, and if it still fails surfaces a clear "transient first-run
  hiccup â please run setup again" message instead of the raw mkcert error.

## [0.7.5] â 2026-07-10

### Fixed
- **First-time setup failed at "Installing mkcert CA" on a fresh Mac**:
  `install_mkcert_ca` ran `mkcert -install` as root via `osascript â¦ with
  ...
  detached root shell doesn't have â so it failed with *"SecTrustSettingsâ¦: The
...
- **A failed setup step marked every later step as failed**: on error the wizard
  drew a red â on all remaining rows even though they never ran. Now only the
...
### Changed
- **Remote Servers: Edit is now a toggle** â clicking Edit again closes the form
  (like Cancel).
...
### Fixed
- **Delete confirmation for remote servers**: deleting a host in Settings â
...
- **UI freezing during Porta Relay operations**: Test connection, the WireGuard
  ...
  commands â which run on the main thread â so the window froze while they waited
...
### Added
- **Porta Relay â expose local apps through your own VPS.** A third expose
...
- **Remote Servers settings section** â register/edit/delete VPS hosts (tunnel IP,
  Caddy admin port, base domain, Mac tunnel IP), with a **Test** button that
  probes the Caddy admin API over the tunnel.
- **Live WireGuard status** â per-host handshake age (green <2 min, amber <5 min,
  red â¥5 min), RX/TX, and endpoint, polled while visible. Exposed apps show an
  amber **degraded** indicator when their tunnel handshake goes stale.
- **Per-route basic auth** on relay routes â reuses an app's existing HTTP basic
  auth, shown with a lock indicator on the public URL.
- **Sync & drift detection** â compare Porta's routes against the live VPS config;
  **Push** restores missing routes and **Remove** drops unmanaged (foreign) ones.
- **Cloudflare DNS auto-record** â opt in per host (with the VPS public IP) to
  auto-create a DNS-only A record for the exposed subdomain on expose.
- **Remote access logs** â tail the VPS Caddy access log over your system SSH
...
- **Docker image update that "succeeds" but keeps reappearing**: the update
  ...
  **multiple** digest aliases, and the stale one can sort first â so the check
...
- **Extension panel wouldn't re-open**: opening an extension from an app card,
  ...
  same card action again did nothing â the focus effect's dependencies were
...
- The app-card files button tooltip is now "Edit config files" (was
  ...
  `.env` â `mise.toml`, `package.json`, `.tool-versions`, and friends too.

## [0.6.36] â 2026-06-18

### Added
- **Generic config file editor**: the per-app file editor now discovers and
  edits more than `.env` and Docker Compose. Recognised config files â
  `mise.toml` / `.mise.toml` / `mise.local.toml`, `.tool-versions`,
  `package.json`, `.nvmrc`, `.ruby-version` â open in a syntax-highlighted code
...
- **Global default upload limit**: a new "Upload size limit" control under
  Settings â Disk Usage sets the default cap (100 MB out of the box, presets up
...
- **64 KB upload cap removed**: Caddy's `request_body` handler was hardcoded to
  64 KB â meant only to bound how much of a body was captured for the Traffic
...
- **Service preset versions**: curated version dropdown per preset (Postgres,
  MySQL, Redis, Mongo, MariaDB, RabbitMQ) via `<datalist>` â users can also
...
- **Stop/cancel for kamal runs**: Stop button in the deploy modal interrupts
  the running command (SIGINT to the process group â SIGKILL after 2 s).
...
- **Command palette (âK)**: scoped search, app + workspace + settings
...
- Command palette: literal ellipsis instead of `â¦` escape.
- Backend: split monolithic `commands.rs` into per-domain modules
  (`app`, `deploy`, `terminal`, `tunnel`, `backup`, â¦); extracted `tray` and
...
- Multi-server kamal deploy with SSL requires a custom certificate (e.g.
  Cloudflare Origin Certificate) â see `docs/plans/` in your app repo.

## [0.1.0-beta] â initial

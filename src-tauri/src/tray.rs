...
pub fn rebuild_tray_menu(app: &tauri::AppHandle, db_path: &Path) {
    ...
    // Disconnect all tunnels — only show when at least one tunnel (Cloudflare
    // or Tailscale) Porta is tracking is active. Cheap checks; no DB query.
    let active_tunnels = crate::commands::active_cloudflared_count()
        + crate::commands::active_serve_count();
    if active_tunnels > 0 {
        let label = format!("⌀ Disconnect All Tunnels ({})", active_tunnels);
        if let Ok(item) = MenuItem::with_id(app, "disconnect-all-tunnels", &label, true, None::<&str>) {
            menu.append(&item).ok();
        }
        PredefinedMenuItem::separator(app).map(|s| menu.append(&s)).ok();
    }
...
pub fn setup_tray(app: &tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    TrayIconBuilder::with_id("porta-main")
        ...
        .on_menu_event(|app: &tauri::AppHandle, event: tauri::menu::MenuEvent| {
            ...
            if id == "show" || id == "header" {
            ...
            } else if id == "disconnect-all-tunnels" {
                let handle = app.clone();
                std::thread::spawn(move || {
                    crate::commands::stop_all_cloudflared_tunnels(&handle);
                    let _ = crate::commands::stop_all_porta_tailscale_serves(handle.clone());
                ...
                std::thread::spawn(move || {
                    ...
                    // Static/proxy apps have no process to toggle — skip.

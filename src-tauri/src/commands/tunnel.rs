...
fn write_tunnel_ingress_config(
    ...
    let service = if has_certs { "https://localhost:443" } else { "http://localhost:80" };
...
#[tauri::command]
pub async fn start_tunnel(id: String, port: u16, app_handle: tauri::AppHandle) -> Result<(), String> {
...
fn start_tunnel_blocking(id: String, port: u16, app_handle: tauri::AppHandle) -> Result<(), String> {
    ...
    let (tunnel_name, tunnel_hostname, is_static_app, static_host, public_pairs, ingress_config_path) = {
        ...
        match app_info {
            Some(a) => {
                ...
                // wire behavior for users who never used multi-host.
                let cfg_path = if pairs.len() > 1 {
                ...
                (a.tunnel_name.clone(), a.tunnel_custom_hostname.clone(), is_static, host, pairs, cfg_path)
            }
            None => (None, None, false, None, Vec::new(), None),
        }
    };

    // For static apps OR multi-host apps, point cloudflared at Caddy (443 if
    // HTTPS certs exist, else 80) and preserve the app's host header so Caddy
    // matches the right route. With ingress_config_path, host header is
    // per-rule in the yml — these CLI args are unused in that branch.
    ...
    if let (Some(port), Some(name)) = (metrics_port, tunnel_name.as_deref().filter(|s| !s.trim().is_empty())) {
        tunnel_metrics_ports().lock().unwrap().insert(name.to_string(), port);
    }
    ...
    thread::spawn(move || {
        ...
        let err_text = if !intentional && exit_code != 0 {
            ...
            if lines.is_empty() {
                ...
                let hint = if lower.contains("trycloudflare") && (lower.contains("deadline exceeded") || lower.contains("timeout") || lower.contains("context deadline")) {
                ...
                Some(match hint { Some(h) => format!("{}{}", joined, h), None => joined })
            }
        } else {
            None
        };
...
#[tauri::command]
pub async fn route_tunnel_dns(tunnel_name: String, hostname: String, overwrite: bool) -> Result<(), String> {
    tauri::async_runtime::spawn_blocking(move || -> Result<(), String> {
        let cf = find_cloudflared().ok_or_else(|| "cloudflared not installed".to_string())?;
...
fn parse_cloudflared_metrics(text: &str) -> TunnelMetrics {
        ...
        match metric_name {
            ...
            "cloudflared_tunnel_response_by_code" => {
                ...
                if labels
                    ...
                    .any(|kv| {
                        let kv = kv.trim();
                        let v = kv.split_once('=').map(|(_, v)| v.trim_matches('"')).unwrap_or("");
            ...
            "cloudflared_tunnel_response_latency_seconds_bucket" => {
                ...
                if let Some(le_part) = labels.split(',').find(|s| s.trim_start().starts_with("le=")) {
    ...
    let mean_ms = if hist_count > 0.0 { (hist_sum / hist_count) * 1000.0 } else { 0.0 };
...
pub async fn tunnel_metrics(tunnel_name: String) -> Result<TunnelMetrics, TunnelMetricsError> {
    ...
    let client = reqwest::Client::builder()
        ...
        .build()
        .map_err(|e| TunnelMetricsError::Scrape { message: e.to_string() })?;
    let resp = client
        .get(&url)
        .send()
        .await
        .map_err(|e| {
            ...
            if msg.contains("connection refused") || msg.contains("ConnectionRefused") {
                TunnelMetricsError::NotRunning { tunnel_name: name.clone() }
            } else {
    ...
    let text = resp.text().await.map_err(|e| TunnelMetricsError::Scrape { message: e.to_string() })?;

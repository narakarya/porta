...
impl Database {
    ...
    pub fn delete_ssh_host(&self, id: &str) -> Result<()> {
        ...
        // A deploy target that points at a deleted host is a dangling promise —
        // an extension reading it would offer to deploy somewhere that no longer
        // exists.
        self.conn.execute("DELETE FROM app_deploy_targets WHERE host_id=?1", params![id])?;
